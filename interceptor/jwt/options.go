...
type ParseTokenFunc func(auth string, ctx *gin.Context) (*jwt.Token, error)
...
func newOptionSet(opts ...Option) *optionSet {
	...
	if reflect.ValueOf(set.Skipper).Pointer() == reflect.ValueOf(defaultSkipper).Pointer() {
		set.Skipper = func(ctx *gin.Context) bool {
...
type optionSet struct {
	...
	ParseTokenFunc func(auth string, c *gin.Context) (*jwt.Token, error)
}
...
func (set *optionSet) defaultParseToken(auth string, ctx *gin.Context) (*jwt.Token, error) {
	token := new(jwt.Token)
	var err error
...
func jwtFromForm(name string) jwtExtractor {
	return func(ctx *gin.Context) (string, error) {
		field := ctx.Request.Form.Get(name)
		if field == "" {
			return "", errJwtMissing.Err
		}
		return field, nil
